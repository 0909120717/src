 ...
 <!-- content start -->
  <div class="admin-content">
 	<div class="up-cf up-padding">
      <div class="up-fl up-cf"><strong class="up-text-primary up-text-lg">TbProduct</strong> / <small>TbProduct</small></div>
    </div>
          ...
          <c:forEach items="${list}" var="rowdata" varStatus="status">
            <tr >
                            <td><input type="checkbox" name="ids" value="${rowdata.id}" /></td>
              													<td title="${rowdata.code}" > <a href="${basePath}${path}view/${rowdata.id}" class="name">${rowdata.code}</a></td>
                									
																			<td >${rowdata.name}</td>
													
																			<td >${rowdata.categoryId}</td>
													
																		<td ><fmt:formatDate value="${rowdata.validDate}" type="both" pattern="yyyy-MM-dd"/></td>
