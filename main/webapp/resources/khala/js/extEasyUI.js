...
$.extend($.fn.validatebox.methods, {
	...
	reduce : function(jq, newposition) {
		return jq.each(function() {
			...
			$(this).addClass("validatebox-text").validatebox(opt);//console.info($(this));
		});
	}
});
*/
	...
	tooltip : function(jq, fields) {
		...
		function bindEvent(jqs) {
			jqs.mouseover(function() {
				if($(this).children('.easyui-tooltip').length==0){
				...
				if($(this).children('.easyui-tooltip').length!=0){
					$(this).children('.easyui-tooltip').tooltip('destroy');
				}
				$(this).tooltip('destroy');
			});			
		}
...
$.extend($.fn.validatebox.defaults.rules, {
	...
	minLength : { // 判断最小长度
		validator : function(value, param) {		
			return dl.strbytelen(value) >= param[0];
		},
		message : '最少输入 {0} 个字符，一个汉字等于两个字节。'
	},
	maxLength:{
		validator : function(value, param) {
		return dl.strbytelen(value) <= param[0];
	},
	message : '最多输入 {0} 个字符，一个汉字等于两个字节。'
	},
	length : {
		validator : function(value, param) {
			var len = dl.strbytelen($.trim(value));//$.trim(value)			
			return len >= param[0] && len <= param[1];
		},
		message : "内容长度介于{0}个字符和{1}个字符之间，一个汉字等于两个字节。"
	},
	lengthNoKongge : {
		validator : function(value, param) {
			var s=$.trim(value);
	...
	chineseEnglishNumber:{
		validator : function(value) {
			return /^[A-Za-z\u0391-\uFFE5\d][\sA-Za-z\u0391-\uFFE5\d\.\_\-]*$/i.test(value);
		},
		message : '不能输入特殊字符'
	},
	space:{//验证是否包含空格
 ...
 * @returns $.modalDialog.xxx 这个xxx是可以自己定义名称，主要用在弹窗关闭时，刷新某些对象的操作，可以将xxx这个对象预定义好
 */
$.modalDialogExt = function(options) {
	var opts = $.extend({
		title : '',
		width : 840,
		height : 680,
...
$.modalDialogExtNo = function(options) {
	var opts = $.extend({
		title : '',
		width : 840,
		height : 680,
 ...
 * datagrid无数据显示
 */
$.datagrid_nodata = function(obj){
	var body = obj.data().datagrid.dc.body2;
	body.find('table tbody').append('<tr><td width="' + body.width() + '" style="height: 25px; color: red; text-align: center;">没有数据</td></tr>');
